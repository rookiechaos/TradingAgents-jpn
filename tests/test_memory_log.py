...
def _structured_pm_llm(captured: dict, decision: PortfolioDecision | None = None):
    ...
    if decision is None:
        decision = PortfolioDecision(
            ...
            investment_judgment="中立",
            confidence="中",
            key_reasons="材料が拮抗している。",
            counterarguments="上方・下方いずれの決め手も弱い。",
            biggest_risk="次回決算で前提が崩れる可能性。",
            next_checkpoints="次回決算とガイダンス更新を確認。",
        )
    ...
    llm.with_structured_output.side_effect = lambda schema: (
        captured.__setitem__("schema", schema) or structured
    )
    return llm
...
class TestPortfolioManagerInjection:
    ...
    def test_pm_returns_rendered_markdown_with_rating(self):
        ...
        decision = PortfolioDecision(
            ...
            confidence="高",
            key_reasons="AI capex cycle and institutional flows remain constructive.",
            counterarguments="Valuation is not cheap and guidance must hold.",
            biggest_risk="Capex slowdown would weaken the thesis quickly.",
            next_checkpoints="Next earnings, guidance, and large-customer demand signals.",
        )
        llm = _structured_pm_llm(captured, decision)
        pm_node = create_portfolio_manager(llm)
        result = pm_node(_make_pm_state())
        md = result["final_trade_decision"]
        assert captured["schema"] is JapanPortfolioDecision
        assert "**Rating**: Overweight" in md
        assert "**投資判断**: 買い" in md
        assert "**確信度**: 高" in md
        assert "**Executive Summary**: Build position gradually" in md
        assert "**Investment Thesis**: AI capex cycle" in md
        assert "**Price Target**: 215.0" in md
        assert "**Time Horizon**: 3-6 months" in md
        assert result["final_trade_decision_structured"]["investment_judgment"] == "買い"
        assert result["final_trade_decision_is_structured"] is True
    ...
    def test_pm_falls_back_to_freetext_when_structured_unavailable(self):
        ...
        assert result["final_trade_decision_is_structured"] is False

    def test_pm_prompt_omits_japanese_committee_instruction_outside_jp_mode(self):
        captured = {}
        before = get_config()
        try:
            set_config({"market_profile": "global"})
            decision = PortfolioDecision(
                rating=PortfolioRating.HOLD,
                executive_summary="Hold and monitor.",
                investment_thesis="Balanced evidence with no clear catalyst.",
            )
            llm = _structured_pm_llm(captured, decision)
            pm_node = create_portfolio_manager(llm)
            result = pm_node(_make_pm_state())
            assert "Always fill the Japanese investment-committee fields" not in captured["prompt"]
            assert captured["schema"] is PortfolioDecision
            assert "**投資判断**" not in result["final_trade_decision"]
        finally:
            set_config(before)
