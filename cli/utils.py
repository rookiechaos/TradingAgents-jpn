    ...
    class _MissingQuestionary:
        def __getattr__(self, name):
            raise ModuleNotFoundError(
                "questionary is required for interactive CLI prompts. "
                "Install project dependencies before running the interactive CLI."
            )
...
TICKER_INPUT_EXAMPLES = "7203.T, 8306, SPY, 0700.HK, BTC-USD"
...
def get_ticker(config: dict | None = None) -> str:
    ...
    return normalize_ticker_symbol(ticker, config=config) if ticker.strip() else "SPY"


def normalize_ticker_symbol(ticker: str, config: dict | None = None) -> str:
    """Normalize ticker input while preserving exchange suffixes.

    In JP-equity profile, bare 4-digit Tokyo Stock Exchange codes
    are auto-normalized to ``.T`` so users can type ``7203`` and still land on
    ``7203.T`` without losing the explicit exchange suffix downstream.
    """
    normalized = ticker.strip().upper()
    cfg = config or get_config() or DEFAULT_CONFIG
    if (
        str(cfg.get("market_profile", "")).strip().lower() == "jp_equity"
        and cfg.get("auto_suffix_for_plain_jp_tickers", False)
        and normalized.isdigit()
        and len(normalized) == 4
    ):
        return f"{normalized}{cfg.get('default_exchange_suffix', '.T')}"
    return normalized
...
def ask_output_language() -> str:
    ...
    choice = questionary.select(
        ...
        choices=[
            questionary.Choice("Japanese (default, 日本語)", "Japanese"),
