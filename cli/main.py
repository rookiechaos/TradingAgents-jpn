...
app = typer.Typer(
    name="TradingAgents",
    help="TradingAgents-jpn CLI: JP Equity Research Copilot",
...
class MessageBuffer:
    ...
    def _update_current_report(self):
        ...
        if latest_section and latest_content:
            ...
            section_titles = {
                ...
                "final_trade_decision": "Investment Committee Recommendation",
            }
            self.current_report = (
    ...
    def _update_final_report(self):
        ...
        if self.report_sections.get("final_trade_decision"):
            report_parts.append("## Investment Committee Recommendation")
...
def update_display(layout, spinner_text=None, stats_handler=None, start_time=None):
    ...
    layout["header"].update(
        Panel(
            ...
            "[bold cyan]JP Equity Research Copilot[/bold cyan]\n"
            "[yellow]投資助言ではありません。Research use only.[/yellow]\n"
...
def get_user_selections():
    ...
    welcome_content += "[bold green]TradingAgents-jpn: JP Equity Research Copilot - CLI[/bold green]\n\n"
    welcome_content += "[yellow]投資助言ではありません。調査補助を目的とした研究専用ワークフローです。[/yellow]\n\n"
    ...
    welcome_box = Panel(
        ...
        padding=(1, 2),
        title="Welcome to TradingAgents",
        subtitle="JP Equity Research Copilot",
    )
...
def resolve_report_benchmark(
    ticker: str,
    final_state: dict | None = None,
    config: dict | None = None,
) -> str:
    """Resolve the benchmark shown in exported reports.

    Preference order:
    1. Explicit benchmark embedded in the final state / log state
    2. Explicit config override via ``benchmark_ticker``
    3. Config benchmark map by ticker suffix
    4. Default US fallback
    """
    if final_state and final_state.get("benchmark_ticker"):
        return final_state["benchmark_ticker"]
    ...
    for suffix, candidate in benchmark_map.items():
...
def save_report_to_disk(final_state, ticker: str, save_path: Path, config: dict | None = None):
    ...
    sections = []
    benchmark = resolve_report_benchmark(ticker, final_state=final_state, config=config)
    ...
    if final_state.get("risk_debate_state"):
        ...
        if risk.get("judge_decision"):
            ...
            sections.append(f"## V. Investment Committee Recommendation\n\n### Portfolio Manager\n{risk['judge_decision']}")

    # Write consolidated report
    header = (
        get_localized_report_header(
            ticker,
            final_state.get("trade_date", datetime.datetime.now().strftime("%Y-%m-%d")),
            benchmark,
            config=config,
        )
        + "\n\n"
...
def display_complete_report(final_state):
    ...
    console.print(f"[yellow]{get_research_only_notice()}[/yellow]")
    ...
    if final_state.get("risk_debate_state"):
        ...
        if risk.get("judge_decision"):
            console.print(Panel("[bold]V. Investment Committee Recommendation[/bold]", border_style="green"))
